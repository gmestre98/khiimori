...
## Acceptance criteria
- [ ] The map renders a pin for the day's `Stay` (if located) and for each located `PlanItem`
  (activity/transport).
- [ ] Pins are ordered by the itinerary `order` (Milestone 04), so their sequence matches the day plan.
- [ ] Pin coordinates come from Epic 02's geocoding/cache (server-side), not client geocoding.
- [ ] Pins may use restrained accent colour per the minimal theme (PRD §5.10).
