...
## Acceptance criteria
- [ ] DB URL, OAuth client, and Maps key are exposed to the container as **Secret Manager-backed** env vars
  (or mounted secrets), referencing the S4 secret **versions** — not literal values.
- [ ] No secret value appears in the Pulumi program, committed config, container image, or CI logs (PRD §8.5).
- [ ] The env var names match what the app's config layer (M01.2 S1) expects.
- [ ] Rotating a secret version updates the running service via `pulumi up` (or the documented version pin) without code changes.
- [ ] `pulumi up` applies cleanly and the service boots reading secrets from the environment.
