...
## Acceptance criteria
- [ ] Runs **only on `main`** (after lint/unit/build/integration pass).
- [ ] Builds the image from the S3 Dockerfile and pushes it to the M01.4 Artifact Registry repo.
- [ ] The image is tagged with the **commit SHA** (and optionally `latest`) so deploys are traceable/rollback-able.
- [ ] Authenticates via **WIF** (S5) — no SA keys; no secrets in logs (PRD §8.5).
- [ ] The pushed image reference (path + SHA tag) is passed to the deploy stage (S7).
