...
## Acceptance criteria
- [ ] A workflow triggers on PRs and pushes to `main`, running both backend (Go) and web (TS) jobs.
- [ ] It runs the **existing** lint/format checks (M01.1 S6/S7) and `go test ./...` + the web unit tests.
- [ ] Go and Node toolchains are pinned to the repo's versions; dependency caching is enabled for speed.
- [ ] A failing lint or unit test **fails the check** (branch-protection-ready, gating the change).
- [ ] Jobs run in parallel where independent to keep wall-clock + CI minutes down.
