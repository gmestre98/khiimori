...
> **Status:** ✅ Done — build stage (go build + vite build) gates the change (#120).
...
## Acceptance criteria
- [x] CI runs `go build ./...` (and `go vet`) for the backend and the web production build (e.g. `vite build`).
- [x] The build stage runs after lint/unit and **fails the check** on any compile/bundle error.
- [x] Build artifacts (or their absence on failure) are observable in the run; caching reused from S1.
- [x] No deploy/push here — purely proving both build (containerisation is S3).
