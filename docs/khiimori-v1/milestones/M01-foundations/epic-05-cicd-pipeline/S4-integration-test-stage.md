...
> **Status:** ✅ Done — integration tests run vs an ephemeral Postgres service in CI (#122).
...
## Acceptance criteria
- [x] A CI stage runs the integration-tagged tests (M01.3 S7) against an **ephemeral Neon branch** (or test DB).
- [x] DB connection details are injected from **GitHub secrets**, never printed to logs (PRD §8.5).
- [x] The stage runs after build and **gates the change** on integration failures.
- [x] The ephemeral branch/DB is **torn down** (or relies on Neon expiry) so branches don't accumulate (PRD §8.1).
- [x] Runs only where appropriate (e.g. PRs + `main`) to conserve CI minutes (PRD §8.4 #4).
