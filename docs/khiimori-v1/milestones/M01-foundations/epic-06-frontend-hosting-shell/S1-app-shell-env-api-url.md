...
## Acceptance criteria
- [ ] A minimal shell renders (app title/layout placeholder) — no real feature screens (those are Milestone 09).
- [ ] The API base URL is read from a **build-time env var** (e.g. `VITE_API_BASE_URL`), with a documented local default.
- [ ] A tiny typed API helper centralises the base URL (one place to change), used by S2's health view.
- [ ] **No hardcoded production URL** anywhere in the source (epic AC4).
- [ ] `.env.example` (or equivalent) documents the variable; real values are not committed.
