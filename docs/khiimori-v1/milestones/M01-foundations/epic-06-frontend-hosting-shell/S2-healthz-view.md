...
## Acceptance criteria
- [ ] The shell calls `GET {API_BASE_URL}/healthz` on load (or via a button) using the S1 API client.
- [ ] It clearly renders the outcome: healthy vs unreachable/error (status shown to the user).
- [ ] Network/error states are handled gracefully (no uncaught promise, no blank screen on failure).
- [ ] A unit/component test mocks the fetch and asserts both the success and failure renderings.
