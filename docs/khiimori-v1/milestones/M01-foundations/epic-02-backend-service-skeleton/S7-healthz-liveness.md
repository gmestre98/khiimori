...
## Acceptance criteria
- [x] `GET /healthz` returns `200 OK` with a small JSON body (e.g. `{ "status": "ok" }`).
- [x] The handler performs **no** I/O to the database or any external dependency.
- [x] Mounted via the shared router so it inherits the S5 middleware chain.
- [x] **Unit tests** assert the 200 status and body using `httptest`.
