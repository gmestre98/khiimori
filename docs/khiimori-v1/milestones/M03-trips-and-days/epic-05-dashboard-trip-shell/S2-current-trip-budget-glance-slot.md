...
## Acceptance criteria
- [ ] When a current trip exists (flagged by Epic 03), it is surfaced **prominently** above/within the
  dashboard.
- [ ] It shows **today's day number** (derived from the trip's days / today).
- [ ] It renders a **budget-glance slot** as a defined placeholder/component boundary that Milestone 05
  fills with real figures — no budget math here.
- [ ] When no current trip exists, the surface degrades gracefully (hidden or a neutral prompt).
