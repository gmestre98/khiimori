...
## Acceptance criteria
- [x] An **archive** action moves a trip out of the active lists (calls Epic 01 S4) and the dashboard
  updates immediately.
- [x] A **delete** action requires explicit confirmation (it cascades days/owned data) and removes the
  trip on confirm.
- [x] Archived trips are visible in the **Past/archived** context (or an archived view), not the active
  buckets.
- [x] Only a user authorized to manage the trip sees these controls (server still enforces — Epic 04).
