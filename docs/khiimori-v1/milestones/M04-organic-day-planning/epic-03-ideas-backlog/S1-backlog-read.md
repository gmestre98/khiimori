...
## Acceptance criteria
- [x] An endpoint returns a trip's `PlanItem`s with `day_id = null` (the backlog), ordered by `order`.
- [x] The read is **authorized** via the M03 `Authorizer`.
- [x] The response is suitable for the day-view/backlog UI (Epic 05) to render and act on.
