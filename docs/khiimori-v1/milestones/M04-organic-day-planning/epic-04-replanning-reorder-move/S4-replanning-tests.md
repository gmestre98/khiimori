...
## Acceptance criteria
- [x] Tests cover **reorder** within a day (sequence updates; convergence-friendly).
- [x] Tests cover **move between days** (day_id change, order placement, same row).
- [x] Tests cover **status transitions** (each value; invalid rejected).
- [x] A test replays the same reorder/move/status mutation twice and asserts **no duplication/corruption**
  (idempotency for Epic 06).
- [x] Integration tests run against the migrated schema (M01.3 harness).
