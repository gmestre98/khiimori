...
## Acceptance criteria
- [x] A documented strategy resolves conflicts deterministically (e.g. last-write-wins by timestamp, or
  field-level merge for independent fields) — chosen approach recorded.
- [x] Reorders converge using the convergence-friendly `order` scheme (Epic 04 S1) rather than clobbering.
- [x] A conflicting replay does not crash or silently lose data beyond the documented rule; the user can
  see the resulting state.
- [x] A unit test exercises at least one conflict case and asserts the deterministic outcome.
