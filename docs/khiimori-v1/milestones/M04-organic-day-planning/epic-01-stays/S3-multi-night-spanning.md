...
## Acceptance criteria
- [x] A day's read includes the stay(s) whose `[check_in, check_out)` range covers that date, derived at
  read time (no duplicate rows per night).
- [x] Editing a stay's dates updates which days it appears on, with no data duplication.
- [x] Unit + integration tests cover: a stay shown on each covered day, a single-night stay, and date-edit
  changing coverage.
- [x] Tests also cover add/edit/remove against a real migrated schema (M01.3 harness).
