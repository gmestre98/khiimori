...
## Acceptance criteria
- [ ] A read returns a user's memberships (the trips they belong to and at what role) — used by the trips
  listing scope (Milestone 03 S2 / Epic 03).
- [ ] A read returns a trip's members (and roles) — used by the sharing UI (Epic 04) and admin (Epic 05).
- [ ] A read answers "what role does user U have on trip T?" for the `Authorizer` (Epic 02).
- [ ] A unit test covers each read shape.
