...
## Acceptance criteria
- [ ] Integration tests cover, **for each module** (Trip, Budget, Journal, Geo): **Owner** (full),
  **Editor** (edit plan/budget/journal, not sharing), **Viewer** (read-only), **non-member** (denied).
- [ ] Tests assert **server-side** enforcement (unauthorized → 403/404, not data) — not hidden UI.
- [ ] Tests cover **revocation** and **role downgrade** taking effect immediately (S3).
- [ ] Tests run against the real membership `Authorizer` (S1/S2), reusing the M01.3 harness.
