...
## Acceptance criteria
- [x] From a trip's sharing surface, an Owner can enter an **email** and pick a **role** (Editor/Viewer)
  and send an invite (Epic 03 S3).
- [x] The surface shows pending invitations and their **status** (sent/accepted).
- [x] Invite controls are visible only to a user with Owner capability (server still enforces — Epic 02).
- [x] The surface is responsive (web + mobile); Milestone 09 components when available.
