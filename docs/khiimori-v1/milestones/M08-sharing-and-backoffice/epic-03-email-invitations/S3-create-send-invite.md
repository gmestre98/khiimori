...
## Acceptance criteria
- [x] An endpoint lets an **Owner** create an invitation for an email + role (Editor/Viewer), generating a
  unique token and `status = sent`.
- [x] The invite email is sent via the `EmailSender` (S2) with an accept link carrying the token.
- [x] Creating an invite is **authorized** (only an Owner of the trip, via the `Authorizer`).
- [x] A unit test covers create + send (faked sender) and that a non-owner cannot invite.
