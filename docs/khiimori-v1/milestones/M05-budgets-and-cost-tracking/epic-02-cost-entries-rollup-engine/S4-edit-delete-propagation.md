...
## Acceptance criteria
- [x] Editing or deleting a `CostEntry` updates the affected category/day/trip roll-ups.
- [x] Editing or deleting a **stay or plan item cost** (Milestone 04) is reflected in the roll-ups (since
  actuals are computed-on-read, this is automatic — verify and test it).
- [x] Roll-up reads are **consistent** within a transaction/request (no torn totals).
- [x] A unit/integration test edits and deletes each source type and asserts the roll-up changes.
