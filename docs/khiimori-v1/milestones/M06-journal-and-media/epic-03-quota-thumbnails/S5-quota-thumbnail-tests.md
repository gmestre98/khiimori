...
## Acceptance criteria
- [x] Integration tests (M01.3 harness, faked/local `MediaStore`) cover **cap enforcement** (under / at /
  over → rejected).
- [x] Tests cover **usage accounting** on add and delete (no drift).
- [x] Tests cover **thumbnail generation** (a variant is produced and associated).
- [x] A test confirms an over-cap upload stores **nothing** (no object, no row, no usage change).
