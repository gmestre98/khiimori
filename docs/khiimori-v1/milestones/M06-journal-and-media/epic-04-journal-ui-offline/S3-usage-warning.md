...
## Acceptance criteria
- [x] The UI shows **per-trip photo usage** (used vs. 1 GB) from Epic 03 S4.
- [x] It **warns** as usage approaches the cap (e.g. a visible indicator/threshold).
- [x] When an upload is **rejected** for exceeding the cap (Epic 03 S2), the server message is surfaced
  clearly with guidance (delete photos / it won't upload).
- [x] The displayed figure matches the server's enforced usage (single source of truth).
