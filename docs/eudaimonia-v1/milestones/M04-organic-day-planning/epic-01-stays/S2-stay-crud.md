...
## Acceptance criteria
- [ ] Endpoints create, edit, and remove a `Stay` scoped to a trip; only `name` is required on create.
- [ ] All operations are **authorized** via the trip `Authorizer` (M03 Epic 04) — only permitted users
  may modify a trip's stays.
- [ ] Mutations are **idempotent/queueable** (stable id / upsert semantics) so Epic 06's offline layer can
  replay them.
- [ ] A unit test covers add/edit/remove and an unauthorized attempt being denied.
