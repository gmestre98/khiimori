...
## Acceptance criteria
- [ ] A `ReadinessCheck` abstraction (a named func returning an error) and a way to register checks at startup.
- [ ] `GET /readyz` runs all registered checks: `200` with a per-check JSON status when all pass; `503` with
  the failing checks named when any fails.
- [ ] With **no** checks registered (today's state) it returns `200 ready`.
- [ ] Checks run with a bounded timeout so a hung dependency can't hang the probe.
- [ ] The contract is stable so M01.3 can register a DB check with no handler/route changes — leave a
  documented seam/TODO marking where the DB check will attach.
- [ ] **Unit tests** cover: no checks → 200, a passing check → 200, and a failing check → 503.
