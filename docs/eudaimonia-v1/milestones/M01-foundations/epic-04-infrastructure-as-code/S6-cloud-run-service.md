...
## Acceptance criteria
- [ ] A Cloud Run (v2) service is created in the configured region, using an image from the S2 registry
  (a placeholder/initial tag is fine until M01.5 pushes real images).
- [ ] It runs as the **S5 service account** (not the default compute SA).
- [ ] Liveness/startup probes point at **`/healthz`**, readiness at **`/readyz`** (M01.2 endpoints).
- [ ] The container `PORT` aligns with the service's config (M01.2 S1) and the service listens on it.
- [ ] The service URL is a Pulumi **stack output** (M01.6 web shell + CORS need it).
- [ ] `run.googleapis.com` enabled via IaC; `pulumi up`/`destroy` work cleanly.
