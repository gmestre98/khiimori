...
## Acceptance criteria
- [ ] Deployed service logs appear in **Cloud Logging** as structured entries (fields queryable, not opaque text).
- [ ] The logger's level maps to Cloud Logging **severity** (so error entries are classified as `ERROR`).
- [ ] Each log entry carries the **request id** (M01.2 S5) and, where available, the Cloud Run trace/correlation field.
- [ ] A documented Logs Explorer query finds all errors for a given request id.
- [ ] Consistent with the project policy that only **error-level** logs are emitted for now (M01.2 S2).
