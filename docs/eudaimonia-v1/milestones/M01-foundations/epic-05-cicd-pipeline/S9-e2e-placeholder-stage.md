...
## Acceptance criteria
- [ ] An e2e stage exists in the pipeline, runnable after deploy, targeting a **staging/preview** URL from config (not prod).
- [ ] It runs a **trivial smoke check** today (e.g. hit `/healthz` and load the web shell) and passes.
- [ ] The stage is structured so Milestone 10 can add the critical-journey tests without re-plumbing
  (env vars, base URLs, and a clear place for specs are in place).
- [ ] It can be made **required** later; for now it gates on the smoke check and is clearly marked a placeholder.
- [ ] Documented: how Milestone 10 will fill in the real journeys.
