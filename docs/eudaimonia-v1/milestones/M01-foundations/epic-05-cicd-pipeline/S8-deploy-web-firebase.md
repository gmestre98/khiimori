...
## Acceptance criteria
- [ ] Runs **only on `main`**, builds the production web bundle, and deploys it to the M01.4 Hosting site.
- [ ] The **API base URL is injected at build time from config/env** (no hardcoded prod URL) (aligns with M01.6 S1).
- [ ] Deploy authenticates without long-lived keys (WIF / Firebase CI token stored as a GitHub secret, not in logs) (PRD §8.5).
- [ ] The deploy is atomic and the resulting Hosting URL is reported in the run.
- [ ] Failure to build or deploy **fails the stage**.
