...
## Acceptance criteria
- [ ] A multi-stage Dockerfile builds a **minimal** runtime image (e.g. distroless/static) running the service.
- [ ] The image runs as a **non-root** user, listens on the configured `PORT` (M01.2 S1), and contains **no secrets**.
- [ ] The base image and Go version are pinned; the build is reproducible.
- [ ] CI builds the image (no push yet) to prove it compiles in a container.
- [ ] Image size is reasonable (documented), and `/healthz`/`/readyz` work in a locally-run container.
