...
## Acceptance criteria
- [ ] **Migrations run before** the new revision serves traffic (using the M01.3 S5 runner against the prod DB via the direct connection).
- [ ] The Cloud Run service is updated to the **SHA-tagged image** from S6 (consistent with the IaC-managed service — no drift).
- [ ] Deploy uses **WIF** auth (S5); the DB/connection secrets come from Secret Manager, never CI logs (PRD §8.5).
- [ ] A failed migration **aborts the deploy** (no half-migrated, mismatched revision).
- [ ] Post-deploy, the new revision passes its `/readyz` check before the stage is green.
