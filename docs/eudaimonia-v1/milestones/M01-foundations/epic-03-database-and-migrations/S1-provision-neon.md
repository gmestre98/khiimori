...
## Acceptance criteria
- [x] A single Neon project + database exists on the **free tier**, in a region close to the GCP region used by M01.4.
- [x] Both the **pooled** (pgBouncer) and **direct** connection strings are identified and documented
  (the driver story S2 needs both).
- [ ] A dedicated application role/user is used (not the Neon admin/owner) for the service connection. _(deferred: uses `neondb_owner`; follow-up documented in `backend/docs/database.md`.)_
- [x] Connection secrets are recorded in a way that is **not committed to git** (e.g. a local `.env`
  ignored by git + a note that M01.4 moves them to Secret Manager).
- [x] `docs/` (or the epic) notes the project/branch names and how to regenerate the password.
