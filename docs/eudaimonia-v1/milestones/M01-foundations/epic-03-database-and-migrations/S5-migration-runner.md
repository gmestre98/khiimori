...
## Acceptance criteria
- [x] One command applies all pending migrations (e.g. `make migrate-up` / `scripts/migrate.ts` / a Go
  subcommand) using the **direct** connection from config.
- [x] A matching command rolls back (or to a version) for local iteration.
- [x] The command exits non-zero with a clear message on failure (so CI can gate on it).
- [x] It targets whatever DB the config/env points at (dev, ephemeral test branch, prod) with no code change.
- [x] Usage is documented (one short section).
