...
## Acceptance criteria
- [x] A `ReadinessCheck` is registered that pings the DB through the **pooler** (the path real traffic uses) (PRD §8.6).
- [x] `GET /readyz` returns `503` (naming the failing `db` check) when the DB is unreachable and `200` when it's healthy.
- [x] `/healthz` is **unaffected** — liveness still does no DB I/O (M01.2 S7).
- [x] The check honours the registry's bounded timeout so a slow/cold Neon instance can't hang the probe.
- [x] A unit test with a fake/failing DB pinger asserts the 200/503 behaviour (no live DB required).
