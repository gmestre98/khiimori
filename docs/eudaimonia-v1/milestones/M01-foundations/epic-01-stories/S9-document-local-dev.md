...
## Acceptance criteria
- [ ] Root `README.md` (and/or `backend/README.md` + `web/README.md`) documents prerequisites
  (Go version, Node version) and the **one-command** startup from S8.
- [ ] Lint, format, and test commands for both Go and the web app are listed.
- [ ] A short "from clone to running" section is included.
- [ ] Steps verified by following them on a fresh clone (note in the PR that this was done).
