...
## Acceptance criteria
- [ ] ESLint configured for TypeScript + React with a committed config.
- [ ] Prettier configured with a committed config; ESLint and Prettier don't fight (formatting rules
  deferred to Prettier).
- [ ] `npm run lint` and `npm run format` (or `format:check`) are defined in `web/package.json` and
  pass on the current tree.
- [ ] Config is minimal — no premature framework/style rules.
