...
## Acceptance criteria
- [ ] Integration tests (M01.3 harness) seed stays, plan items, and cost entries across categories/days
  and assert correct **per-category, per-day, per-trip** totals.
- [ ] Tests cover **category mapping** for each source type.
- [ ] Tests cover **edit/delete propagation** for each source (totals change correctly).
- [ ] A test covers trip-vs-day budget interaction (per-day lines plus a trip-level line).
