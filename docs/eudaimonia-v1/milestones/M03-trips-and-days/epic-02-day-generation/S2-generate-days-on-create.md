...
## Acceptance criteria
- [ ] On trip create, exactly **one `Day` per date** in `[start_date, end_date]` is created, with a stable
  `index` (e.g. 1..N) and the real `date`.
- [ ] Generation runs in the **same transaction** as trip create (Epic 01 S2) — trip + days commit
  together.
- [ ] Generation is **idempotent** against the `(trip_id, date)` guard (re-running does not duplicate
  days).
- [ ] A unit test covers a multi-day trip and a single-day trip (one day generated).
