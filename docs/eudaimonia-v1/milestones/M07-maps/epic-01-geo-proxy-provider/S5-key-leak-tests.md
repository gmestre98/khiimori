...
## Acceptance criteria
- [ ] A test asserts the Maps API key (and any privileged token) **never appears** in proxy responses,
  logs, or errors.
- [ ] A test asserts client-facing endpoints go through the proxy/provider interface (provider faked) and
  cannot be made to return the key.
- [ ] A test covers that a referer-locked client key, if used (S4), is the restricted key — or that no
  client key is shipped at all.
- [ ] Tests run without live Google calls (provider mocked).
