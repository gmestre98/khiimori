...
// handleListCostEntries handles GET /trips/{tripID}/cost-entries. It returns the
// trip's manually-logged cost entries (ad-hoc expenses) ordered by created_at so
// the budget UI can render and edit them; the roll-up already aggregates them.
func (m *Module) handleListCostEntries(w http.ResponseWriter, r *http.Request) {
	...
	out := make([]costEntryResponse, len(entries))
