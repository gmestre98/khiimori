...
// whole history costs a few MB of cache at most (photos are excluded). Map tiles
...
// the maps work offline. Returns nothing; failures are swallowed.
async function prewarmDay(trip: Trip, date: string, signal?: AbortSignal): Promise<void> {
  try {
    ...
    await warmDayRoute(trip.id, date, collectLocations(day), signal)
  } catch {
...
// and every one of its days.
async function prewarmTrip(trip: Trip, signal?: AbortSignal): Promise<void> {
  await Promise.all([
  ...
  await runPool(dates, (date) => prewarmDay(trip, date, signal))
}

// prefetchAllTripsForOffline pre-warms every trip's data for offline use. Call
// it once the user is authenticated; it self-limits to a single run per session
// and returns immediately (doing its work in the background) when it should skip.
// Never throws.
